...
public class WebGPUApplicationConfiguration extends WebGPUWindowConfiguration {
	...
	void set (WebGPUApplicationConfiguration config) {
	...
	public static DisplayMode getDisplayMode () {
		WebGPUApplication.initializeGlfw();
		GLFWVidMode videoMode = GLFW.glfwGetVideoMode(GLFW.glfwGetPrimaryMonitor());
		return new WebGPUGraphics.WebGPUDisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(), videoMode.height(),
	...
	public static DisplayMode getDisplayMode (Monitor monitor) {
		WebGPUApplication.initializeGlfw();
		GLFWVidMode videoMode = GLFW.glfwGetVideoMode(((WebGPUGraphics.WebGPUMonitor)monitor).monitorHandle);
		return new WebGPUGraphics.WebGPUDisplayMode(((WebGPUGraphics.WebGPUMonitor)monitor).monitorHandle, videoMode.width(), videoMode.height(),
			videoMode.refreshRate(), videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
	}
	...
	public static DisplayMode[] getDisplayModes () {
		...
		for (int i = 0; i < result.length; i++) {
			...
			result[i] = new WebGPUGraphics.WebGPUDisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(), videoMode.height(),
				videoMode.refreshRate(), videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
		}
		return result;
	}
	...
	public static DisplayMode[] getDisplayModes (Monitor monitor) {
		WebGPUApplication.initializeGlfw();
		Buffer videoModes = GLFW.glfwGetVideoModes(((WebGPUGraphics.WebGPUMonitor)monitor).monitorHandle);
		DisplayMode[] result = new DisplayMode[videoModes.limit()];
		for (int i = 0; i < result.length; i++) {
			GLFWVidMode videoMode = videoModes.get(i);
			result[i] = new WebGPUGraphics.WebGPUDisplayMode(((WebGPUGraphics.WebGPUMonitor)monitor).monitorHandle, videoMode.width(),
	...
	static WebGPUGraphics.WebGPUMonitor toWebGPUMonitor (long glfwMonitor) {
		...
		return new WebGPUGraphics.WebGPUMonitor(glfwMonitor, virtualX, virtualY, name);
	}

	static GridPoint2 calculateCenteredWindowPosition (WebGPUGraphics.WebGPUMonitor monitor, int newWidth, int newHeight) {
		IntBuffer tmp = BufferUtils.createIntBuffer(1);
		IntBuffer tmp2 = BufferUtils.createIntBuffer(1);
