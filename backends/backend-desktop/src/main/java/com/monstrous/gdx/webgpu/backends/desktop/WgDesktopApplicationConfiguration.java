...
public class WgDesktopApplicationConfiguration extends WgDesktopWindowConfiguration {
    ...
    /**
     * Whether to disable audio or not. If set to true, the returned audio class instances like {@link Audio} or
     * {@link Music} will be mock implementations.
     */
    ...
    /**
     * Set transparent window hint. Results may vary on different OS and GPUs. Usage with the ANGLE backend is less
     * consistent.
     */
    ...
    /**
     * Sets the target framerate for the application. The CPU sleeps as needed. Must be positive. Use 0 to never sleep.
     * Default is 0.
     */
    ...
    /**
     * Sets the directory where {@link Preferences} will be stored, as well as the file type to be used to store them.
     * Defaults to "$USER_HOME/.prefs/" and {@link FileType#External}.
     */
    ...
    /**
     * Defines how HDPI monitors are handled. Operating systems may have a per-monitor HDPI scale setting. The operating
     * system may report window width/height and mouse coordinates in a logical coordinate system at a lower resolution
     * than the actual physical resolution. This setting allows you to specify whether you want to work in logical or
     * raw pixel units. See {@link HdpiMode} for more information. Note that some OpenGL functions like
     * {@link GL20#glViewport(int, int, int, int)} and {@link GL20#glScissor(int, int, int, int)} require raw pixel
     * units. Use {@link HdpiUtils} to help with the conversion if HdpiMode is set to {@link HdpiMode#Logical}. Defaults
     * to {@link HdpiMode#Logical}.
     */
    ...
    public static DisplayMode getDisplayMode() {
        ...
        return new WgDesktopGraphics.WebGPUDisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(),
    ...
    public static DisplayMode getDisplayMode(Monitor monitor) {
        ...
        return new WgDesktopGraphics.WebGPUDisplayMode(((WgDesktopGraphics.WebGPUMonitor) monitor).monitorHandle,
    ...
    public static DisplayMode[] getDisplayModes() {
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            result[i] = new WgDesktopGraphics.WebGPUDisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(),
                    videoMode.height(), videoMode.refreshRate(),
                    videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
        }
        return result;
    ...
    public static DisplayMode[] getDisplayModes(Monitor monitor) {
        ...
        for (int i = 0; i < result.length; i++) {
            GLFWVidMode videoMode = videoModes.get(i);
            result[i] = new WgDesktopGraphics.WebGPUDisplayMode(
                    ((WgDesktopGraphics.WebGPUMonitor) monitor).monitorHandle, videoMode.width(), videoMode.height(),
    ...
    static GridPoint2 calculateCenteredWindowPosition(WgDesktopGraphics.WebGPUMonitor monitor, int newWidth,
            int newHeight) {
        IntBuffer tmp = BufferUtils.createIntBuffer(1);
        IntBuffer tmp2 = BufferUtils.createIntBuffer(1);
        ...
        // If the new width is greater than the working area, we have to ignore stuff like the taskbar for centering and
        // use the
