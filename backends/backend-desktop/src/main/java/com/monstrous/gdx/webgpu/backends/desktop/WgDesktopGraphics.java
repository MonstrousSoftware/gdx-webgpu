...
public class WgDesktopGraphics implements WgGraphics, Disposable {
    ...
    public boolean setWindowedMode(int width, int height) {
        ...
        if (!isFullscreen()) {
            ...
            if (width != logicalWidth || height != logicalHeight) {
                ...
                newPos = WgDesktopApplicationConfiguration.calculateCenteredWindowPosition((WebGPUMonitor) getMonitor(),
                        width, height);
            }
            GLFW.glfwSetWindowSize(window.getWindowHandle(), width, height);
            if (centerWindow) {
                window.setPosition(newPos.x, newPos.y); // on macOS the centering has to happen _after_ the new window
                                                        // size was set
            }
            ...
            if (width != windowWidthBeforeFullscreen || height != windowHeightBeforeFullscreen) { // center the window
                                                                                                  // since its size
                // changed
                GridPoint2 newPos = WgDesktopApplicationConfiguration
                        .calculateCenteredWindowPosition((WebGPUMonitor) getMonitor(), width, height);
                ...
                GLFW.glfwSetWindowMonitor(window.getWindowHandle(), 0, windowPosXBeforeFullscreen,
                        windowPosYBeforeFullscreen, width, height, displayModeBeforeFullscreen.refreshRate);
            }
        }
        updateFramebufferInfo();
        return true;
    ...
    public void setUndecorated(boolean undecorated) {
        ...
        GLFW.glfwSetWindowAttrib(window.getWindowHandle(), GLFW.GLFW_DECORATED,
    ...
    public void setResizable(boolean resizable) {
        ...
        GLFW.glfwSetWindowAttrib(window.getWindowHandle(), GLFW.GLFW_RESIZABLE,
    ...
    /**
     * Sets the target framerate for the application, when using continuous rendering. Must be positive. The cpu sleeps
     * as needed. Use 0 to never sleep. If there are multiple windows, the value for the first window created is used
     * for all. Default is 0.
     *
     * @param fps fps
     */
    @Override
