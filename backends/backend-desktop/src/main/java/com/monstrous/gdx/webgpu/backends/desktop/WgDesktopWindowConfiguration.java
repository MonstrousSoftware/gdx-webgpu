...
public class WgDesktopWindowConfiguration {
    ...
    /**
     * @param autoIconify whether the window should automatically iconify and restore previous video mode on input focus
     *            loss. (default true) Does nothing in windowed mode.
     */
    ...
    /**
     * Sets the position of the window in windowed mode. Default -1 for both coordinates for centered on primary
     * monitor.
     */
    ...
    /**
     * Sets minimum and maximum size limits for the window. If the window is full screen or not resizable, these limits
     * are ignored. The default for all four parameters is -1, which means unrestricted.
     */
     ...
     * icons.
     * 
     * @param filePaths One or more {@linkplain FileType#Internal internal} image paths. Must be JPEG, PNG, or BMP
     *            format. The one closest to the system's desired size will be scaled. Good sizes include 16x16, 32x32
     *            and 48x48.
     */
     ...
     * @param filePaths One or more image paths, relative to the given {@linkplain FileType}. Must be JPEG, PNG, or BMP
     *            format. The one closest to the system's desired size will be scaled. Good sizes include 16x16, 32x32
     *            and 48x48.
     */
    ...
    /**
     * Sets the {@link WgDesktopWindowListener} which will be informed about iconficiation, focus loss and window close
     * events.
     */
    ...
    /**
     * Sets the app to use fullscreen mode. Use the static methods like getDisplayMode() on this class to enumerate
     * connected monitors and their fullscreen display modes.
     */
     ...
     *
     * For multi-window applications, only one (the main) window should enable vsync. Otherwise, every window will wait
     * for the vertical blank on swap individually, effectively cutting the frame rate to (refreshRate /
     * numberOfWindows).
     */
