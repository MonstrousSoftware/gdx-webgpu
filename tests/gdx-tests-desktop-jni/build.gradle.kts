...
tasks.register<JavaExec>("gdx_webgpu_tests_desktop_jni_wgpu_run") {
    ...
    classpath = sourceSets["main"].runtimeClasspath + configurations.detachedConfiguration(
        dependencies.create(
            "com.github.xpenatan.jWebGPU:webgpu-desktop-jni-wgpu_$currentDesktopPlatform:$jWebGPUVersion"
        )
    )
...
tasks.register<JavaExec>("gdx_webgpu_tests_desktop_jni_dawn_run") {
    ...
    classpath = sourceSets["main"].runtimeClasspath + configurations.detachedConfiguration(
        dependencies.create(
            "com.github.xpenatan.jWebGPU:webgpu-desktop-jni-dawn_$currentDesktopPlatform:$jWebGPUVersion"
        )
    )
    workingDir = File("../assets")
    setIgnoreExitValue(true)
