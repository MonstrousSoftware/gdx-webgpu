...
/**
 * This is a test class, showing how one could implement a height field. See also {@link HeightMapTest}. Do not expect
 * this to be a fully supported and implemented height field class.
 * <p />
 * Represents a HeightField, which is an evenly spaced grid of values, where each value defines the height on that
 * position of the grid, so forming a 3D shape. Typically used for (relatively simple) terrains and such. See
 * <a href="http://en.wikipedia.org/wiki/Heightmap">wikipedia</a> for more information.
 * <p />
 * A height field has a width and height, specifying the width and height of the grid. Points on this grid are specified
 * using integer values, named "x" and "y". Do not confuse these with the x, y and z floating point values representing
 * coordinates in world space.
 * <p />
 * The values of the heightfield are normalized. Meaning that they typically range from 0 to 1 (but they can be negative
 * or more than one). The plane of the heightfield can be specified using the {@link #corner00}, {@link #corner01},
 * {@link #corner10} and {@link #corner11} members. Where `corner00` is the location on the grid at x:0, y;0, `corner01`
 * at x:0, y:height-1, `corner10` at x:width-1, y:0 and `corner11` the location on the grid at x:width-1, y:height-1.
 * <p />
 * The height and direction of the field can be set using the {@link #magnitude} vector. Typically this should be the
 * vector perpendicular to the heightfield. E.g. if the field is on the XZ plane, then the magnitude is typically
 * pointing on the Y axis. The length of the `magnitude` specifies the height of the height field. In other words, the
 * word coordinate of a point on the grid is specified as:
 * <p />
 ...
 * <p />
 * You can set this heightfield using the constructor or one of the `set` methods. E.g. by specifying an array of values
 * or a {@link Pixmap}. The latter can be used to load a HeightMap, which is an image loaded from disc of which each
 * texel is used to specify the value for each point on the field. Be aware that the total number of vertices cannot
 * exceed 32k. Using a large height map will result in unpredicted results.
 * <p />
 * You can also manually modify the heightfield by directly accessing the {@link #data} member. The index within this
 * array can be calculates as: `y * width + x`. E.g. `field.data[y * field.width + x] = value;`. When you modify the
 * data then you can update the {@link #mesh} using the {@link #update()} method.
 * <p />
 * The {@link #mesh} member can be used to render the height field. The vertex attributes this mesh contains are
 * specified in the constructor. There are two ways for generating the mesh: smooth and sharp.
 * <p />
 * Smooth can be forced by specifying `true` for the `smooth` argument of the constructor. Otherwise it will be based on
 * whether the specified vertex attributes contains a normal attribute. If there is no normal attribute then the mesh
 * will always be smooth (even when you specify `false` in the constructor). In this case the number of vertices is the
 * same as the amount of grid points. Causing vertices to be shared amongst multiple faces.
 * <p />
 * Sharp will be used if the vertex attributes contains a normal attribute and you didnt specify `true` for the `smooth`
 * argument of the constructor. This will cause the number of vertices to be around four times the amount grid points
 * and each normal is estimated for each face instead of each point.
 * 
 * @author Xoppa
 */
