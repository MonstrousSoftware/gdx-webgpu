...
public final class WgScreenUtils {
    ...
    /**
     * Clears the color buffers, optionally the depth buffer and whether to apply antialiasing (requires to set number
     * of samples in the launcher class).
     *
     * @param clearDepth Clears the depth buffer if true.
    ...
    public static void clear(float r, float g, float b, float a, boolean clearDepth, boolean applyAntialiasing) {
        ...
        WebGPURenderPass renderPass = RenderPassBuilder.create("ScreenUtils.clear", backgroundColor, clearDepth,
                gfx.getContext().getSamples());
        renderPass.end();
        ...
        // if (applyAntialiasing && Gdx.graphics.getBufferFormat().coverageSampling) mask = mask |
        // GL20.GL_COVERAGE_BUFFER_BIT_NV;
        // Gdx.gl.glClear(mask);
    }

    // /** Returns the current framebuffer contents as a {@link TextureRegion} with a width and height equal to the
    // current screen
    // * size. The base {@link Texture} always has {@link MathUtils#nextPowerOfTwo} dimensions and RGBA8888 {@link
    // Format}. It can be
    // * accessed via {@link TextureRegion#getTexture}. The texture is not managed and has to be reloaded manually on a
    // context loss.
    ...
    // }
    //
    // /** Returns a portion of the current framebuffer contents specified by x, y, width and height as a {@link
    // TextureRegion} with
    // * the same dimensions. The base {@link Texture} always has {@link MathUtils#nextPowerOfTwo} dimensions and
    // RGBA8888
    // * {@link Format}. It can be accessed via {@link TextureRegion#getTexture}. This texture is not managed and has to
    // be reloaded
    // * manually on a context loss. If the width and height specified are larger than the framebuffer dimensions, the
    // Texture will
    ...
    // The byte[]
    // * will always contain RGBA8888 data. Because of differences in screen and image origins the framebuffer contents
    // should be
    // * flipped along the Y axis if you intend save them to disk as a bitmap. Flipping is not a cheap operation, so use
    // this
    ...
    // }
    //
    // /** Returns a portion of the current framebuffer contents specified by x, y, width and height, as a byte[] array
    // with a length
    // * equal to the specified width * height * 4. The byte[] will always contain RGBA8888 data. If the width and
    // height specified
    // * are larger than the framebuffer dimensions, the Texture will be padded accordingly. Pixels that fall outside of
    // the current
    // * screen will have RGBA values of 0. Because of differences in screen and image origins the framebuffer contents
    // should be
    // * flipped along the Y axis if you intend save them to disk as a bitmap. Flipping is not a cheap operation, so use
    // this
    // * functionality wisely.
