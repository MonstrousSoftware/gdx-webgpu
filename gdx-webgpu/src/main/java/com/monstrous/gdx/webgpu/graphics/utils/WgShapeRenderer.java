...
public class WgShapeRenderer implements Disposable {
    ...
    public WgShapeRenderer(int maxVertices) {
        this(maxVertices, null);
    }

    public WgShapeRenderer(int maxVertices, ShaderProgram defaultShader) {
        if (defaultShader == null) {
            renderer = new WgImmediateModeRenderer(maxVertices, false, true, 0);
        } else {
            renderer = new WgImmediateModeRenderer(maxVertices, false, true, 0, defaultShader);
        }
    ...
    /**
     * If true, when drawing a shape cannot be performed with the current shape type, the batch is flushed and the shape
     * type is changed automatically. This can increase the number of batch flushes if care is not taken to draw the
    ...
    /**
     * Draws a line using {@link ShapeType#Line} or {@link ShapeType#Filled}. The line is drawn with two colors
    ...
    /**
     * Draws a line using a rotated rectangle, where with one edge is centered at x1, y1 and the opposite edge centered
     * at x2, y2.
     */
    ...
    /**
     * Draws a cube using {@link ShapeType#Line} or {@link ShapeType#Filled}. The x, y and z specify the bottom, left,
     * front corner of the rectangle.
     */
    ...
    /**
     * Calls {@link #arc(float, float, float, float, float, int)} by estimating the number of segments needed for a
     * smooth arc.
     */
    ...
    /**
     * Calls {@link #ellipse(float, float, float, float, float, int)} by estimating the number of segments needed for a
     * smooth ellipse.
     */
    ...
    /**
     * Calls {@link #cone(float, float, float, float, float, int)} by estimating the number of segments needed for a
    ...
    /**
     * Draws a polygon in the x/y plane using {@link ShapeType#Line}. The vertices must contain at least 3 points (6
     * floats x,y).
     */
    ...
    /**
     * Draws a polyline in the x/y plane using {@link ShapeType#Line}. The vertices must contain at least 2 points (4
     * floats x,y).
     */
    ...
    /**
     * Checks whether the correct ShapeType was set. If not and autoShapeType is enabled, it flushes the batch and
     * changes the shape type. The batch is also flushed, when the matrix has been changed or not enough vertices
     * remain.
     *
