...
/**
 * Version of Stage for WebGPU. Just makes sure we use a WebGPUSpriteBatch and a WebGPUViewport.
 */
public class WgStage extends Stage {
    ...
    /**
     * Creates a stage with a {@link ScalingViewport} set to {@link Scaling#stretch}. The stage will use its own
    ...
    public WgStage() {
        super(new ScalingViewport(Scaling.stretch, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(),
                new OrthographicCamera()), new WgSpriteBatch());
    }

    /**
     * Creates a stage with the specified viewport. The stage will use its own {@link Batch} which will be disposed when
     * the stage is disposed.
     */
    ...
    public void draw() {
        ...
        // There is a static boolean debug in Stage that indicates if any actor has ever needed debug draw, to avoid
        // calling drawDebug() unless needed.
        // However we cannot access this package private static member. We add stage.enableDebug() to block debug
        // drawing including the recursive descent.
    ...
    /**
     * Allow debug drawing (default is enabled). Set false to disable any debug lines, for a small performance boost
     */
    ...
    private void drawActorDebug(Actor actor, WgShapeRenderer debugShapes) {
        ...
        debugShapes.rect(stageX, stageY, actor.getOriginX(), actor.getOriginY(), width, height, actor.getScaleX(),
                actor.getScaleY(), actor.getRotation());
    }
