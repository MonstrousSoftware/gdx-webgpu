 ...
 *
 * Usage without dynamic offsets: buffer.set(0, transformMatrix); // set uniform value (a matrix) at offset 0
 * buffer.set(64, diffuseColor); // set uniform value, offset in bytes buffer.flush();
 *
 * Usage with dynamic offset: buffer.beginSlices(); // reset dynamic offset int dynamicOffset = buffer.nextSlice(); //
 * start a new slice buffer.set(0, transformMatrix); // set uniform value buffer.set(64, diffuseColor); // set uniform
 * value, offset in bytes, relative to this slice binder.bindGroup(renderPass, 3, dynamicOffset); // pass dynamic offset
 * to the render pass
 *
 * dynamicOffset = buffer.nextSlice(); // next slice buffer.set(0, transformMatrix); // buffer.set(64, diffuseColor); //
 * buffer.endSlices(); // write content to GPU! binder.bindGroup(renderPass, 3, dynamicOffset);
 *
...
public class WebGPUUniformBuffer extends WebGPUBuffer {
     ...
     *
     * @param contentSize size of data per slice in bytes (will be padded to a valid uniform stride size) Note: data for
     *            each slice doesn't have to be the same size. Provide the average or worst case size to allow
    ...
    /**
     * Flush data and start a new set of data. The data can be set using the set() methods. Returns dynamic offset for
     * this set of data.
     */
