...
public class WgGLTFModelLoader extends WgModelLoader<WgModelLoader.ModelParameters> {
    ...
    private void loadMaterials(ModelData modelData, GLTF gltf) {
        ...
        for (GLTFMaterial gltfMat : gltf.materials) {
            ...
            modelMaterial.id = gltfMat.name != null ? gltfMat.name : "mat" + index; // copy name or generate one to be
                                                                                    // used as reference
            index++;
            ...
            // modelMaterial.roughnessFactor = gltfMat.pbrMetallicRoughness.roughnessFactor;
            // if(gltfMat.pbrMetallicRoughness.metallicFactor >= 0)
            // modelMaterial.metallicFactor = gltfMat.pbrMetallicRoughness.metallicFactor;
            ...
            if (gltfMat.normalTexture >= 0) {
                GLTFImage image = gltf.images.get(gltf.textures.get(gltfMat.normalTexture).source);
                PBRModelTexture tex = new PBRModelTexture();
            ...
            if (gltfMat.emissiveTexture >= 0) {
                GLTFImage image = gltf.images.get(gltf.textures.get(gltfMat.emissiveTexture).source);
                PBRModelTexture tex = new PBRModelTexture();
            ...
            if (gltfMat.occlusionTexture >= 0) {
                GLTFImage image = gltf.images.get(gltf.textures.get(gltfMat.occlusionTexture).source);
                PBRModelTexture tex = new PBRModelTexture();
    ...
    private void loadTexture(GLTF gltf, PBRModelTexture tex, GLTFImage image, int textureId, boolean isColor) {
        if (image.uri != null) {
            ...
            tex.fileName = "bufferView." + image.bufferView; // create a unique 'filename' that can be used as key for
                                                             // caching

            Pixmap pixmap = new Pixmap(bytes, 0, view.byteLength);
            // System.out.println("Creating pixmap from binary chunk: "+tex.fileName+"
    ...
    Texture.TextureFilter convertFilter(int glFilter) {
        ...
        switch (glFilter) {
            ...
            case GL20.GL_LINEAR:
                ...
                break;
            case GL20.GL_LINEAR_MIPMAP_LINEAR:
                filter = Texture.TextureFilter.MipMap;
                break;
            case GL20.GL_NEAREST_MIPMAP_NEAREST:
                filter = Texture.TextureFilter.MipMapNearestNearest;
                break;
            case GL20.GL_LINEAR_MIPMAP_NEAREST:
                filter = Texture.TextureFilter.MipMapLinearNearest;
                break;
            case GL20.GL_NEAREST_MIPMAP_LINEAR:
                filter = Texture.TextureFilter.MipMapNearestLinear;
                break;
            default:
                filter = Texture.TextureFilter.Nearest;
                break;
    ...
    Texture.TextureWrap convertWrap(int glWrap) {
        ...
        switch (glWrap) {
            case GL20.GL_MIRRORED_REPEAT:
                wrap = Texture.TextureWrap.MirroredRepeat;
                break;
            case GL20.GL_CLAMP_TO_EDGE:
    ...
    private ModelNode addNode(ModelData modelData, GLTF gltf, GLTFNode gltfNode) {
        ...
        if (gltfNode.mesh >= 0) { // this node refers to a mesh
            ...
            for (GLTFPrimitive primitive : gltfMesh.primitives) {
                ...
                // System.out.println("Node refers to mesh part :" + nodePart.meshPartId + " material:
                // "+nodePart.materialId);

                // if this primitive is rigged (vertex attributes include JOINTS_0) add a bones array to the node part
                // (will be filled in loadSkins())
        ...
        for (GLTFPrimitive primitive : gltfMesh.primitives) {
            ...
            if (jointsAccessorId >= 0) {
                ...
                if (jointsAccessor.componentType != GLTF.USHORT16 && jointsAccessor.componentType != GLTF.UBYTE8)
                    throw new RuntimeException("GLTF: Can only joints defined as USHORT16 or UBYTE8, type = "
            ...
            if (weightsAccessorId >= 0) {
                ...
                if (accessor.componentType != GLTF.FLOAT32 || !accessor.type.contentEquals("VEC4"))
                    throw new RuntimeException(
                            "GLTF: Can only support vec4(FLOAT32) for joints, type = " + accessor.componentType);
                view = gltf.bufferViews.get(accessor.bufferView);
                // if (view.buffer != 0)
                // throw new RuntimeException("GLTF: Can only support buffer 0");
                rawBuffer = gltf.rawBuffers.get(view.buffer);
