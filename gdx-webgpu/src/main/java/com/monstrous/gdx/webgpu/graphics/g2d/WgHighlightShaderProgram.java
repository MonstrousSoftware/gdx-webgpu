...
/**
 * Custom shader program for sprite highlighting. Renders an outline around non-transparent pixels.
 * The highlight follows the actual shape of the sprite (respecting alpha channel), not the texture bounds.
 *
...
public class WgHighlightShaderProgram extends WgShaderProgram {
    ...
    /**
     * Create a highlight shader with the specified outline thickness.
     * Uses default alpha threshold of 0.1 and no smoothing.
    ...
    /**
     * Create a highlight shader with the specified outline thickness and alpha threshold.
     * Uses no smoothing by default.
     * @param thickness outline width in pixels
     * @param alphaThreshold alpha threshold (0.0 to 1.0) - pixels with alpha below this are considered transparent.
     *                       Lower values (e.g., 0.01) create tighter outlines, higher values (e.g., 0.3) include semi-transparent edges.
     */
     ...
     * @param thickness outline width in pixels
     * @param alphaThreshold alpha threshold (0.0 to 1.0) - pixels with alpha below this are considered transparent.
     * @param smoothing smoothing amount (0.0 to 3.0) - higher values create smoother, anti-aliased edges.
     *                  0.0 = hard edges (pixelated), 1.0-2.0 = smooth edges (recommended), 3.0+ = very soft edges.
     */
    public WgHighlightShaderProgram(float thickness, float alphaThreshold, float smoothing) {
        super("highlight shader",
              generateShaderSource(thickness, alphaThreshold, smoothing),
              getShaderPrefix());
    ...
    private static String generateShaderSource(float thickness, float alphaThreshold, float smoothing) {
        ...
        return "// Outline shader - converted from OpenGL\n" +
                ...
                "    let width = " + thickness + "; // u_width\n" +
                "    let pattern = 1; // pattern 1 = circular\n" +
                "    let alphaThreshold = " + alphaThreshold + "; // Configurable alpha threshold\n" +
                "    \n" +
                ...
                "    \n" +
                "    let alphaThreshold = " + alphaThreshold + ";\n" +
                "    let width = " + thickness + ";\n" +
