...
/**
 * A skin stores resources for UI widgets to use (texture regions, ninepatches, fonts, colors, etc). Resources are named
 * and can be looked up by name and type. Resources can be described in JSON. Skin provides useful conversions, such as
 * allowing access to regions in the atlas as ninepatches, sprites, drawables, etc. The get* methods return an instance
 * of the object in the skin. The new* methods return a copy of an instance in the skin.
 * <p>
...
public class WgSkin extends Skin {
    ...
    public WgSkin(FileHandle skinFile) {
        ...
        if (atlasFile.exists()) {
            atlas = new WgTextureAtlas(atlasFile);
            addRegions(atlas);
        }
    ...
    /**
     * Creates a skin containing the resources in the specified skin JSON file and the texture regions from the
     * specified atlas. The atlas is automatically disposed when the skin is disposed.
     */
    public WgSkin(FileHandle skinFile, TextureAtlas atlas) {
        this.atlas = atlas;
        addRegions(atlas);
        load(skinFile);
    }
    ...
    public void load(FileHandle skinFile) {
        try {
            getJsonLoader(skinFile).fromJson(WgSkin.class, skinFile);
    ...
    /**
     * Adds all named texture regions from the atlas. The atlas will not be automatically disposed when the skin is
     * disposed.
     */
    ...
    /**
     * Returns a registered texture region. If no region is found but a texture exists with the name, a region is
     * created from the texture and stored in the skin.
     */
    ...
    /**
     * Returns a registered tiled drawable. If no tiled drawable is found but a region exists with the name, a tiled
     * drawable is created from the region and stored in the skin.
     */
    ...
    /**
     * Returns a registered ninepatch. If no ninepatch is found but a region exists with the name, a ninepatch is
     * created from the region and stored in the skin. If the region is an {@link AtlasRegion} then its split
     * {@link AtlasRegion#values} are used, otherwise the ninepatch will have the region as the center patch.
     */
    ...
    /**
     * Returns a registered sprite. If no sprite is found but a region exists with the name, a sprite is created from
     * the region and stored in the skin. If the region is an {@link AtlasRegion} then an {@link AtlasSprite} is used if
     * the region has been whitespace stripped or packed rotated 90 degrees.
     */
    ...
    /**
     * Returns a registered drawable. If no drawable is found but a region, ninepatch, or sprite exists with the name,
     * then the appropriate drawable is created and stored in the skin.
     */
    public Drawable getDrawable(String name) {
        ...
        // Use texture or texture region. If it has splits, use ninepatch. If it has rotation or whitespace stripping,
        // use sprite.
        try {
            TextureRegion textureRegion = getRegion(name);
            if (textureRegion instanceof AtlasRegion) {
    ...
    /**
     * Returns the name of the specified style object, or null if it is not in the skin. This compares potentially every
     * style object in the skin of the same type as the specified style, which may be a somewhat expensive operation.
     */
     ...
     * <p>
     * This can be useful when scaling an entire UI (eg with a stage's viewport) then using an atlas with images whose
     * resolution matches the UI scale. The skin can then be scaled the opposite amount so that the larger or smaller
     * images are drawn at the original size. For example, if the UI is scaled 2x, the atlas would have images that are
     * twice the size, then the skin's scale would be set to 0.5.
     */
    ...
    /**
     * Sets the style on the actor to disabled or enabled. This is done by appending "-disabled" to the style name when
     * enabled is false, and removing "-disabled" from the style name when enabled is true. If the style was not found
     * in the skin, an exception is thrown.
     */
    ...
    /**
     * Sets the style on the actor to disabled or enabled. This is done by appending "-disabled" to the style name when
     * enabled is false, and removing "-disabled" from the style name when enabled is true. A method named "getStyle" is
     * called the actor via reflection and the name of that style is found in the skin. If the actor doesn't have a
     * "getStyle" and "setStyle" method the actor is left unchanged. If the style was not found in the skin, an
     * exception is thrown.
     */
    @Deprecated
    ...
    protected Json getJsonLoader(final FileHandle skinFile) {
        ...
        json.setSerializer(WgSkin.class, new ReadOnlySerializer<WgSkin>() {
            public WgSkin read(Json json, JsonValue typeToValueMap, Class ignored) {
    ...
    /**
     * Returns a map of {@link Json#addClassTag(String, Class) class tags} that will be used when loading skin JSON. The
     * map can be modified before calling {@link #load(FileHandle)}. By default the map is populated with the simple
