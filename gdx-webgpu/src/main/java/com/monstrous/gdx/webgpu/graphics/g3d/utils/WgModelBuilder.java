...
/**
 * Derived class from ModelBuilder to use WebGPUMeshBuilder instead of regular MeshBuilder. Because model and node are
 * private some methods that refer to them had to be duplicated as well.
 *
 */
public class WgModelBuilder extends ModelBuilder {
    ...
    private Array<WgMeshBuilder> builders = new Array<WgMeshBuilder>();

    private WgMeshBuilder getBuilder(final VertexAttributes attributes) {
        for (final WgMeshBuilder mb : builders)
    ...
    /**
     * Adds the {@link Node} to the model and sets it active for building. Use any of the part(...) method to add a
     * NodePart.
     */
    ...
    /**
     * Adds the specified MeshPart to the current Node. The Mesh will be managed by the model and disposed when the
     * model is disposed. The resources the Material might contain are not managed, use {@link #manage(Disposable)} to
