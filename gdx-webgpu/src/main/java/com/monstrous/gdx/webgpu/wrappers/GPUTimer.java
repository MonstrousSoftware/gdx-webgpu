...
public class GPUTimer implements Disposable {
    ...
    /**
     * get a query set that can be made into a query for renderPassDescriptor.setTimestampWrites(query); Will be null if
    ...
    public void fetchTimestamps() {
        ...
        timeStampMapBuffer.mapAsync(WGPUMapMode.Read, 0, 8 * 2 * MAX_PASSES, WGPUCallbackMode.AllowProcessEvents,
                callback);
    }
    ...
    public float getAverageGPUtime(int pass) {
        if (numSamples[pass] == 0)
            return 0;
        float avg = (float) (cumulative[pass] / 1000) / (float) numSamples[pass]; // average time converted from nano to
                                                                                  // microseconds
