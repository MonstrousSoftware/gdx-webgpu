...
public class WebGPUPipeline implements Disposable {
    ...
    public WebGPUPipeline(WGPUPipelineLayout pipelineLayout, PipelineSpecification spec) {
        ...
        // if the specification does not already have a shader, create one from the source file, customized to the
        // vertex attributes.
        if (spec.shader != null) {
            shader = spec.shader; // make use of the shader in the spec
            ownsShader = false; // we don't have to dispose it
        } else {
            String prefix = ShaderPrefix.buildPrefix(spec.vertexAttributes, spec.environment, spec.maxDirLights,
                    spec.maxPointLights, spec.usePBR);
        ...
        pipelineDesc.getPrimitive()
                .setStripIndexFormat(isStripTopology(spec.topology) ? spec.indexFormat : WGPUIndexFormat.Undefined);
        ...
        if (spec.isSkyBox) {
            ...
            if (!spec.useDepthTest) {
                ...
                if (spec.afterDepthPrepass) { // rely on Z pre-pass
                    ...
                    // this is the usual depth compare: smaller values are closer, near plane has z = 0.0 and far plane
                    // has z = 1.0
