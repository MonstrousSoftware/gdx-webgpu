...
val mainClassName = "com.monstrous.gdx.benchmarks.graalvm.GraalVMWebGPUFfmBenchmarkLauncher"
val nativeImageName = "benchmark-webgpu-graalvm-ffm"
...
dependencies {
    ...
    runtimeOnly(
        "com.github.xpenatan.jWebGPU:" +
            "webgpu-desktop-ffm-${webgpuImplementation.lowercase()}_$currentDesktopPlatform:$jWebGPUVersion"
    )
...
tasks.register<JavaExec>("benchmarkJvm") {
    group = "benchmark"
    description = "Run WebGPU benchmark through FFM on a GraalVM JVM"
    dependsOn("classes")
...
graalvmNative {
    ...
    agent {
        ...
        metadataCopy {
            ...
            outputDirectories.add("src/main/resources/META-INF/native-image/gdx-webgpu/benchmark-graalvm-ffm")
...
val copyNativeLibrariesToReleaseNativeCompile = tasks.register<Copy>("copyNativeLibrariesToReleaseNativeCompile") {
    ...
    group = "benchmark"
    description = "Copy native libraries next to the GraalVM WebGPU FFM benchmark executable"
...
val copyBenchmarkAssetsToReleaseNativeCompile = tasks.register<Copy>("copyBenchmarkAssetsToReleaseNativeCompile") {
    ...
    group = "benchmark"
    description = "Copy benchmark assets next to the GraalVM WebGPU FFM benchmark executable"
...
tasks.register("buildRelease") {
    group = "benchmark"
    description = "Build the optimized GraalVM WebGPU FFM benchmark executable and copy runtime files"
    dependsOn(copyBenchmarkAssetsToReleaseNativeCompile)
}

tasks.register("benchmarkRelease") {
    group = "benchmark"
    description = "Build and run the optimized GraalVM WebGPU FFM benchmark executable"
    dependsOn(copyBenchmarkAssetsToReleaseNativeCompile)
    ...
    doLast {
        ...
        if(!executable.isFile) {
            throw GradleException("Expected GraalVM WebGPU FFM benchmark executable was not built: ${executable.absolutePath}")
        }
        ...
        if(!completed) {
            ...
            throw GradleException("GraalVM WebGPU FFM benchmark timed out after ${timeoutSeconds}s")
        }
        outputThread.join(1000)
        if(process.exitValue() != 0) {
            throw GradleException("GraalVM WebGPU FFM benchmark failed with exit code ${process.exitValue()}")
        }
    }
}
